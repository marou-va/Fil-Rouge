...
public class CommandeService {
    ...
    public List<Commande> findAll(String statusStr, String dateStr) {
        Statut status = parseStatut(statusStr);
        LocalDateTime[] plage = parsePlageJour(dateStr);
        return commandeDAO.findAll(status, plage[0], plage[1]);
    }
   ...
   //recuperer les information detailer d une commande 
    ...
    public Commande findById(String commandeIdStr) {
        if (commandeIdStr == null || commandeIdStr.isBlank()) {
            return null;
        }
        try {
            return findById(Long.parseLong(commandeIdStr));
   ...
   //annuler une commande 
    ...
    public void updateStatutAdmin(String commandeIdStr, String action, String statutStr) {
        Long commandeId = parseId(commandeIdStr, "Identifiant de commande invalide.");

        if ("ship".equals(action)) {
            commandeDAO.updateStatut(commandeId, Statut.EN_COURS);
            return;
        }

        Statut statut = parseStatut(statutStr);
        if (statut == null) {
            throw new IllegalArgumentException("Statut de commande invalide.");
        }
        commandeDAO.updateStatut(commandeId, statut);
    }
