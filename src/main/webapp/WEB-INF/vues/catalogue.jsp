                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="list-group list-group-flush">
                                    <a href="catalogue"
                                        class="list-group-item list-group-item-action border-0 d-flex justify-content-between align-items-center ${empty param.cid ? 'active bg-brand text-white' : ''}">
                                        <span>Toutes les catégories</span>
                                        <span class="badge ${empty param.cid ? 'bg-white text-brand' : 'bg-light text-muted'} rounded-pill">${totalCount}</span>
                                    </a>
                                    <c:forEach var="cat" items="${categories}">
                                        <a href="catalogue?cid=${cat.id}${not empty param.search ? '&search='.concat(param.search) : ''}"
                                            class="list-group-item list-group-item-action border-0 d-flex justify-content-between align-items-center ${param.cid == cat.id ? 'active bg-brand text-white' : ''}">
                                            <span>${cat.nom}</span>
                                            <span class="badge ${param.cid == cat.id ? 'bg-white text-brand' : 'bg-light text-muted'} rounded-pill">${categoryCounts[cat.id]}</span>
                                        </a>
                                    </c:forEach>
                                </div>
