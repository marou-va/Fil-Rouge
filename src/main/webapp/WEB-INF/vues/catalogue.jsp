                    ...
                    <div class="col-lg-3">
                        ...
                        <div class="card-theme p-3 mb-4">
                            <h6 class="fw-bold mb-3 d-flex align-items-center gap-2">
                                <span
                                    ...
                                    <i class="fas fa-euro-sign text-white" style="font-size:.75rem;"></i>
                                </span>
                                Prix (MAD)
                            </h6>
                            <form action="catalogue" method="GET">
                                ...
                                <div class="row g-2 mb-3">
                                    <div class="col-6">
                                        <input type="number" name="minPrice" class="form-control form-control-sm" placeholder="Min" value="${minPrice}">
                                    </div>
                                    <div class="col-6">
                                        <input type="number" name="maxPrice" class="form-control form-control-sm" placeholder="Max" value="${maxPrice}">
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-brand btn-sm w-100">Filtrer</button>
                                <c:if test="${not empty minPrice or not empty maxPrice}">
                                    <a href="catalogue?cid=${param.cid}&search=${param.search}&sort=${param.sort}" class="btn btn-link btn-sm w-100 mt-1 text-decoration-none">Réinitialiser</a>
                                </c:if>
                            </form>
                        </div>
                        ...
                        <div class="card-theme p-3">
                            ...
                            <select class="form-select"
                                onchange="window.location.href='catalogue?cid=${param.cid}&search=${param.search}&minPrice=${param.minPrice}&maxPrice=${param.maxPrice}&sort=' + this.value">
                    ...
                    <div class="col-lg-9">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <small class="text-muted">
                                <c:choose>
                                    <c:when test="${totalMatching > 0}">
                                        Affichage de ${((currentPage - 1) * 12) + 1} à ${((currentPage - 1) * 12) + listeProduits.size()} sur ${totalMatching} produits
                                    </c:when>
                                    <c:otherwise>0 produit</c:otherwise>
                                </c:choose>
                            </small>
                        </div>
                            ...
                            <nav aria-label="Page navigation" class="mt-5">
                                <ul class="pagination justify-content-center">
                                    <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                        <a class="page-link" href="catalogue?page=${currentPage - 1}${not empty param.cid ? '&cid='.concat(param.cid) : ''}${not empty param.search ? '&search='.concat(param.search) : ''}${not empty param.sort ? '&sort='.concat(param.sort) : ''}${not empty param.minPrice ? '&minPrice='.concat(param.minPrice) : ''}${not empty param.maxPrice ? '&maxPrice='.concat(param.maxPrice) : ''}" aria-label="Précédent">
                                            <span aria-hidden="true">&laquo;</span>
                                        </a>
                                    </li>
                                    <c:forEach begin="1" end="${totalPages}" var="i">
                                        <li class="page-item ${currentPage == i ? 'active' : ''}">
                                            <a class="page-link" href="catalogue?page=${i}${not empty param.cid ? '&cid='.concat(param.cid) : ''}${not empty param.search ? '&search='.concat(param.search) : ''}${not empty param.sort ? '&sort='.concat(param.sort) : ''}${not empty param.minPrice ? '&minPrice='.concat(param.minPrice) : ''}${not empty param.maxPrice ? '&maxPrice='.concat(param.maxPrice) : ''}">${i}</a>
                                        </li>
                                    </c:forEach>
                                    <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                        <a class="page-link" href="catalogue?page=${currentPage + 1}${not empty param.cid ? '&cid='.concat(param.cid) : ''}${not empty param.search ? '&search='.concat(param.search) : ''}${not empty param.sort ? '&sort='.concat(param.sort) : ''}${not empty param.minPrice ? '&minPrice='.concat(param.minPrice) : ''}${not empty param.maxPrice ? '&maxPrice='.concat(param.maxPrice) : ''}" aria-label="Suivant">
